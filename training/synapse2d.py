...
from utils.train_synapse import train_synapse
from utils.metrics_synapse import evaluate_synapse
...
EXPERIMENT_NAME = 'synapse2d_adam'
...
BASE       = '/home/litisnouman/Desktop/Thesis_18_02_26/datasets/Synapse2D'
TRAIN_PATH = os.path.join(BASE, 'Train')
VALID_PATH = os.path.join(BASE, 'Valid')
TEST_PATH  = os.path.join(BASE, 'Test')

train_images = [os.path.join(TRAIN_PATH, 'images', i) for i in os.listdir(os.path.join(TRAIN_PATH, 'images'))]
train_masks  = [i.replace('images', 'masks') for i in train_images]

valid_images = [os.path.join(VALID_PATH, 'images', i) for i in os.listdir(os.path.join(VALID_PATH, 'images'))]
valid_masks  = [i.replace('images', 'masks') for i in valid_images]

test_images  = [os.path.join(TEST_PATH,  'images', i) for i in os.listdir(os.path.join(TEST_PATH,  'images'))]
test_masks   = [i.replace('images', 'masks') for i in test_images]
...
train_dataset = SynapseDataset(train_images, train_masks, transform=train_transform, return_path=False)
valid_dataset = SynapseDataset(valid_images, valid_masks, transform=valid_transform, return_path=False)
valid_data    = test_images[:]  # image paths used by evaluate_synapse / train_synapse
...
#   evaluate_only : if True, only runs evaluation on valid_data (requires model_path)
# ==============================================================================
...
for cfg in models_config:
    ...
    if evaluate_only:
        ...
        model.eval()
        avg_dice = evaluate_synapse(model=model, image_paths=valid_data, num_classes=NUM_CLASSES, transform=evaluate_transform, device=DEVICE, label_dict=LABEL_DICT)
    ...
    train_losses, valid_losses, valid_dice, best_model = train_synapse(
        model=model,
        optimizer=optimizer,
