...
DATASET_MODE     = 'binary'
EXPERIMENT_NAME  = f'animal_{DATASET_MODE}'

# Dataset
NUM_CLASSES      = 3    # foreground pet, background, border (Oxford-IIIT trimap)
IMG_SIZE         = 224
IN_CHANNELS      = 3
...
LABEL_DICT       = None  # e.g. {'Cat': 1, 'Dog': 2}
...
# Known bad / empty-mask images to exclude
TO_REMOVE = [
...
# Stratified split: uppercase filename → cat (1), lowercase → dog (2)
labels_all = [1 if os.path.basename(i)[0].isupper() else 2 for i in images_paths]
print(f'Label distribution (all): {Counter(labels_all)}')

train_valid_images, test_images,  train_valid_masks, test_masks  = train_test_split(images_paths, masks_paths, test_size=0.33, random_state=42, stratify=labels_all)

labels_tv = [1 if os.path.basename(i)[0].isupper() else 2 for i in train_valid_images]
train_images,       valid_images, train_masks,       valid_masks = train_test_split(train_valid_images, train_valid_masks, test_size=0.33, random_state=42, stratify=labels_tv)
...
train_dataset = AnimalDataset(train_images, train_masks, mode=DATASET_MODE, transform=train_transform, return_path=False)
valid_dataset = AnimalDataset(valid_images, valid_masks, mode=DATASET_MODE, transform=valid_transform, return_path=False)
